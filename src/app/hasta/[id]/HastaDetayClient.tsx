...
export default function HastaDetayClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const hastaRes = await fetch(`/api/hastalar/${id}`);
        if (!hastaRes.ok) throw new Error('Hasta bilgileri alınamadı');
        const hastaData = await hastaRes.json();
        setHasta(hastaData);

        // Tedavi planlarını al
        const tedaviRes = await fetch(`/api/hastalar/${id}/tedaviler`);
        if (!tedaviRes.ok) throw new Error('Tedavi planları alınamadı');
        const { data: tedaviData } = await tedaviRes.json();
        setTedaviler(tedaviData || []);

        // Ödemeleri al
        const odemeRes = await fetch(`/api/hastalar/${id}/odeme`);
        if (!odemeRes.ok) throw new Error('Ödemeler alınamadı');
        const { data: odemeData } = await odemeRes.json();
  ...
  const handleDurumDegistir = async (tedaviId: number, yeniDurum: string) => {
    try {
      ...
      setTedaviler(prev =>
        prev.map(tedavi =>
          tedavi.id === tedaviId ? { ...tedavi, durum: data.durum } : tedavi
        )
      );
    } catch (err) {
  ...
  const handleTedaviSil = async (tedaviId: number) => {
    try {
      ...
      console.error('Tedavi silme hatası:', err);
      setError('Tedavi planı silinirken bir hata oluştu');
    }
  };

  const handleIndirimGuncelle = async () => {
    const yeniIndirim = window.prompt(
      'Toplam indirimli tutarı girin (İndirimi kaldırmak için boş bırakın):'
    );

    if (yeniIndirim === null) return; // İptal edildi
    ...
    try {
      const response = await fetch(`/api/hastalar/${id}/indirim`, {
        ...
        body: JSON.stringify({
          toplamIndirim: yeniIndirim === '' ? null : parseFloat(yeniIndirim),
        }),
      });

      if (!response.ok) {
        throw new Error('İndirim güncellenemedi');
      }

      const { data } = await response.json();
      setHasta(prevHasta => prevHasta ? { ...prevHasta, toplamIndirim: data.toplamIndirim } : null);
    } catch (err) {
      console.error('İndirim güncelleme hatası:', err);
      setError('İndirim güncellenirken bir hata oluştu');
    }
  };
  ...
  const toplamFiyat = tedaviler.reduce((acc, t) => acc + t.fiyat, 0);
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                <p><span className="font-medium">T.C. Kimlik:</span> {hasta.tcKimlik}</p>
                <p><span className="font-medium">Doğum Tarihi:</span> {new Date(hasta.dogumTarihi).toLocaleDateString('tr-TR')}</p>
                <p><span className="font-medium">Telefon:</span> {hasta.telefon}</p>
                <p><span className="font-medium">E-posta:</span> {hasta.email || "-"}</p>
              </div>
            </div>
            <div>
              <h2 className="text-lg font-semibold mb-4">İletişim Bilgileri</h2>
              <div className="space-y-2">
                <p><span className="font-medium">Adres:</span> {hasta.adres || "-"}</p>
              </div>
            </div>
          </div>
        </div>
